...
#if 1
int vorbis_dsp_init_tremolo(vorbis_dsp_state *v,vorbis_info *vi){
  int i;
...
vorbis_dsp_state *vorbis_dsp_create(vorbis_info *vi){
  ...
  vorbis_dsp_init_tremolo(v,vi);
  return v;
}

void vorbis_dsp_clear_tremolo(vorbis_dsp_state *v){
  int i;
  if(v){
    vorbis_info *vi=v->vi;
...
int vorbis_dsp_pcmout(vorbis_dsp_state *v,ogg_int16_t *pcm,int samples){
  ...
  if(v->out_begin>-1 && v->out_begin<v->out_end){
    ...
    if(pcm){
      ...
      for(i=0;i<vi->channels;i++)
	mdct_unroll_lap_tremolo(ci->blocksizes[0],ci->blocksizes[1],
...
long vorbis_packet_blocksize(vorbis_info *vi,ogg_packet *op){
  ...
  if(oggpack_read_tremolo(&opb,1)!=0){
  ...
  mode=oggpack_read_tremolo(&opb,modebits);
...
int vorbis_dsp_synthesis(vorbis_dsp_state *vd,ogg_packet *op,int decodep){
  ...
  oggpack_readinit_tremolo(&vd->opb,op->packet);

  /* Check the packet type */
  if(oggpack_read_tremolo(&vd->opb,1)!=0){
    /* Oops.  This is not an audio data packet */
    return OV_ENOTAUDIO ;
  }

  /* read our mode and pre/post windowsize */
  mode=oggpack_read_tremolo(&vd->opb,ilog(ci->modes));
  ...
  for(i=0;i<vi->channels;i++)
    mdct_shift_right_tremolo(ci->blocksizes[vd->lW],vd->work[i],vd->mdctright[i]);
  
  if(vd->W){
    int temp;
    oggpack_read_tremolo(&vd->opb,1);
    temp=oggpack_read_tremolo(&vd->opb,1);
    if(temp==-1) return OV_EBADPACKET;
  }
  
  /* packet decode and portions of synthesis that rely on only this block */
  if(decodep){
    mapping_inverse_tremolo(vd,ci->map_param+ci->mode_param[mode].mapping);
