...
void floor1_free_info_tremolo(vorbis_info_floor *i){
...
vorbis_info_floor *floor1_info_unpack_tremolo (vorbis_info *vi,oggpack_buffer *opb){
  codec_setup_info     *ci=(codec_setup_info *)vi->codec_setup;
  int j,k,count=0,maxclass=-1,rangebits;

  vorbis_info_floor1 *info=(vorbis_info_floor1 *)_ogg_calloc(1,sizeof(*info));
  /* read partitions */
  info->partitions=oggpack_read_tremolo(opb,5); /* only 0 to 31 legal */
  info->partitionclass=
    (char *)_ogg_malloc(info->partitions*sizeof(*info->partitionclass));
  for(j=0;j<info->partitions;j++){
    info->partitionclass[j]=(char)oggpack_read_tremolo(opb,4); /* only 0 to 15 legal */
  ...
  for(j=0;j<maxclass+1;j++){
    info->class[j].class_dim=(char)oggpack_read_tremolo(opb,3)+1; /* 1 to 8 */
    info->class[j].class_subs=(char)oggpack_read_tremolo(opb,2); /* 0,1,2,3 bits */
    if(oggpack_eop_tremolo(opb)<0) goto err_out;
    if(info->class[j].class_subs)
      info->class[j].class_book=(unsigned char)oggpack_read_tremolo(opb,8);
    else
      info->class[j].class_book=0;
    if(info->class[j].class_book>=ci->books)goto err_out;
    for(k=0;k<(1<<info->class[j].class_subs);k++){
      info->class[j].class_subbook[k]=(unsigned char)oggpack_read_tremolo(opb,8)-1;
  ...
  /* read the post list */
  info->mult=oggpack_read_tremolo(opb,2)+1;     /* only 1,2,3,4 legal now */
  rangebits=oggpack_read_tremolo(opb,4);
  ...
  for(j=0,k=0;j<info->partitions;j++){
    count+=info->class[info->partitionclass[j]].class_dim;
    for(;k<count;k++){
      int t=info->postlist[k+2]=(ogg_uint16_t)oggpack_read_tremolo(opb,rangebits);
...
static
#endif
int render_point_tremolo(int x0,int x1,int y0,int y1,int x){
...
static void render_line_tremolo(int n,int x0,int x1,int y0,int y1,ogg_int32_t *d){
  int dy;
  int adx;
  int ady;
  int base;
...
int floor1_memosize_tremolo(vorbis_info_floor *i){
  vorbis_info_floor1 *info=(vorbis_info_floor1 *)i;
  return info->posts;
}
...
ogg_int32_t *floor1_inverse1_tremolo(vorbis_dsp_state *vd,vorbis_info_floor *in,
  ...
  if(oggpack_read_tremolo(&vd->opb,1)==1){
    fit_value[0]=oggpack_read_tremolo(&vd->opb,ilog(quant_q-1));
    fit_value[1]=oggpack_read_tremolo(&vd->opb,ilog(quant_q-1));
    ...
    for(i=0,j=2;i<info->partitions;i++){
      ...
      if(csubbits){
	cval=vorbis_book_decode_tremolo(books+info->class[classv].class_book,&vd->opb);
	...
	cval>>=csubbits;
	if(book!=0xff){
	  if((fit_value[j+k]=vorbis_book_decode_tremolo(books+book,&vd->opb))==-1)
	    goto eop;
	}else{
	  fit_value[j+k]=0;
	}
    ...
    for(i=2;i<info->posts;i++){
      int predicted=render_point_tremolo(info->postlist[info->loneighbor[i-2]],
...
int floor1_inverse2_tremolo(vorbis_dsp_state *vd,vorbis_info_floor *in,
  ...
  if(fit_value){
	...
	render_line_tremolo(n,lx,hx,ly,hy,out);
