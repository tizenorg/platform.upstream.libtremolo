...
void oggpack_readinit_tremolo(oggpack_buffer *b,ogg_reference *r){
...
long oggpack_look_tremolo(oggpack_buffer *b,int bits){
    long l;
...
void oggpack_adv_tremolo(oggpack_buffer *b,int bits){
...
/* bits <= 32 */
long oggpack_read_tremolo(oggpack_buffer *b,int bits){
    long l;
...
int oggpack_eop_tremolo(oggpack_buffer *b){
  int ret;
...
long oggpack_bytes_tremolo(oggpack_buffer *b){
  long ret;
...
long oggpack_bits_tremolo(oggpack_buffer *b){
  long ret;
...
static void _span_tremolo(oggpack_buffer *b){
...
void oggpack_adv_tremolo(oggpack_buffer *b,int bits){
  ...
  b->headend-=(bits>>3);
  b->headptr+=(bits>>3);
  if(b->headend<1)_span_tremolo(b);
...
/* bits <= 32 */
long oggpack_read_tremolo(oggpack_buffer *b,int bits){
  long ret=oggpack_look_tremolo(b,bits);
  oggpack_adv_tremolo(b,bits);
  return(ret);
}
...
void cliptest(unsigned long *b,int vals,int bits,int *comp,int compsize){
  long i,bitcount=0;
  ogg_reference *or=ogg_buffer_alloc_tremolo(bs,64);
  for(i=0;i<compsize;i++)
    or->buffer->data[i]= comp[i];
  or->length=i;

  oggpack_readinit_tremolo(&r,or);
  for(i=0;i<vals;i++){
    unsigned long test;
    int tbit=bits?bits:ilog(b[i]);
    if((test=oggpack_look_tremolo(&r,tbit))==0xffffffff)
      report("out of data!\n");
    if(test!=(b[i]&mask[tbit])){
      fprintf(stderr,"%ld) %lx %lx\n",i,(b[i]&mask[tbit]),test);
      report("looked at incorrect value!\n");
    }
    if((test=oggpack_read_tremolo(&r,tbit))==0xffffffff){
    ...
    if(bitcount!=oggpack_bits_tremolo(&r))
      report("wrong number of bits while reading!\n");
    if((bitcount+7)/8!=oggpack_bytes_tremolo(&r))
      report("wrong number of bytes while reading!\n");

  }
  if(oggpack_bytes_tremolo(&r)!=(bitcount+7)/8){
      fprintf(stderr, "%d vs %d\n", oggpack_bytes_tremolo(&r), (bitcount+7)/8);
      report("leftover bytes after read!\n");
  }
  ogg_buffer_release_tremolo(or);
}
...
void _end_verify(int count){
  ...
  int leftover=count*8-oggpack_bits_tremolo(&o);
  if(leftover>7)
    report("\nERROR: too many bits reported left over.\n");

  /* does reading to exactly byte alignment *not* trip EOF? */
  if(oggpack_read_tremolo(&o,leftover)==-1)
    report("\nERROR: read to but not past exact end tripped EOF.\n");
  if(oggpack_bits_tremolo(&o)!=count*8)
    report("\nERROR: read to but not past exact end reported bad bitcount.\n");

  /* does EOF trip properly after a single additional bit? */
  if(oggpack_read_tremolo(&o,1)!=-1)
...
void _end_verify2(int count){
  ...
  /* does reading to exactly byte alignment *not* trip EOF? */
  oggpack_adv_tremolo(&o,leftover);
#ifdef ARM_LITTLE_ENDIAN
...
ogg_reference *ogg_buffer_extend(ogg_reference *or,long bytes){
  if(or){
    ...
    or->next=ogg_buffer_alloc_tremolo(or->buffer->ptr.owner,bytes);
    return(or->next);
  }
  return 0;
}
...
void ogg_buffer_posttruncate(ogg_reference *or,long pos){
  ...
  if(or){
    ogg_buffer_release_tremolo(or->next);
    or->next=0;
    or->length=pos;
  }
}
...
int main(void){
  ...
  or=ogg_buffer_alloc_tremolo(bs,128);
  ...
  for(i=0;i<test2size;i++){
    unsigned long test;
    if((test=oggpack_look_tremolo(&r,32))==0xffffffffUL)report("out of data. failed!");
    if(test!=large[i]){
      fprintf(stderr,"%ld != %ld (%lx!=%lx):",test,large[i],
              test,large[i]);
      report("read incorrect value!\n");
    }
    oggpack_adv_tremolo(&r,32);
  }
  ogg_buffer_release_tremolo(or);
  if(oggpack_bytes_tremolo(&r)!=test2size*4){
    fprintf(stderr, "%d vs %d\n", oggpack_bytes_tremolo(&r), test2size*4);
    report("leftover bytes after read!\n");
  }
  fprintf(stderr,"ok.");
  ...
  fprintf(stderr,"\nTesting read past end (LSb): ");
  {
    ...
    oggpack_readinit_tremolo(&r,&lor);
    for(i=0;i<64;i++){
      if(oggpack_read_tremolo(&r,1)<0){
        fprintf(stderr,"failed; got -1 prematurely.\n");
        exit(1);
      }
    }
    if(oggpack_look_tremolo(&r,1)!=-1 ||
       oggpack_read_tremolo(&r,1)!=-1){
  ...
  }
  {
    ...
    oggpack_readinit_tremolo(&r,&lor);
    if((test=oggpack_read_tremolo(&r,30))==0xffffffffUL ||
       (test=oggpack_read_tremolo(&r,16))==0xffffffffUL){
      fprintf(stderr,"failed 2; got -1 prematurely.\n");
      exit(1);
    }

    if((test=oggpack_look_tremolo(&r,18))==0xffffffffUL){
      fprintf(stderr,"failed 3; got -1 prematurely.\n");
      exit(1);
    }
    if((test=oggpack_look_tremolo(&r,19))!=0xffffffffUL){
      fprintf(stderr,"failed; read past end without -1.\n");
      exit(1);
    }
    if((test=oggpack_look_tremolo(&r,32))!=0xffffffffUL){
      fprintf(stderr,"failed; read past end without -1.\n");
      exit(1);
    }
  }
  ...
  for(i=0;i<10000;i++){
       ...
       byte starting offset within the length of the vector */
    {
      ...
      while(count2){
        ...
        if(or)
          ...
          or=orl=ogg_buffer_alloc_tremolo(bs,64);
	...
	for(k=0;k<ilen;k++)
...

      {
        ...
        for(j=0;j<begin;j++)
          bitcount+=len[j];
        or=ogg_buffer_pretruncate_tremolo(or,bitcount/8);
        ...
        oggpack_readinit_tremolo(&o,or);

        /* verify bit count */
        if(oggpack_bits_tremolo(&o)!=0){
          fprintf(stderr,"\nERROR: Read bitcounter not zero!\n");
          exit(1);
        }
        if(oggpack_bytes_tremolo(&o)!=0){
          fprintf(stderr,"\nERROR: Read bytecounter not zero!\n");
          exit(1);
        }

        bitcount=bitoffset;
        oggpack_read_tremolo(&o,bitoffset);

        /* read and compare to original list */
        for(j=begin;j<begin+ilen;j++){
	  temp=oggpack_read_tremolo(&o,len[j]);
...
,
          ...
          bitcount+=len[j];
          if(oggpack_bits_tremolo(&o)!=bitcount){
            fprintf(stderr,"\nERROR: Read bitcounter %d != %ld!\n",
                    bitcount,oggpack_bits_tremolo(&o));
            exit(1);
          }
          if(oggpack_bytes_tremolo(&o)!=(bitcount+7)/8){
            fprintf(stderr,"\nERROR: Read bytecounter %d != %ld!\n",
                    (bitcount+7)/8,oggpack_bytes_tremolo(&o));
            exit(1);
          }
        ...
        oggpack_readinit_tremolo(&o,or);
        bitcount=bitoffset;
        oggpack_adv_tremolo(&o,bitoffset);

        /* read and compare to original list */
        for(j=begin;j<begin+ilen;j++){
	  temp=oggpack_look_tremolo(&o,len[j]);
