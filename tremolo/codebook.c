...
/**** pack/unpack helpers ******************************************/
int _ilog_tremolo(unsigned int v){
  int ret=0;
  while(v){
    ret++;
    v>>=1;
...
static ogg_uint32_t decpack_tremolo(long entry,long used_entry,long quantvals,
  ...
  case 1:
	...
	ret|=((ogg_uint16_t *)(b->q_val))[off]<<(b->q_bits*j);
      ...
      for(j=0;j<b->dim;j++)
	ret|=oggpack_read_tremolo(opb,b->q_bits)<<(b->q_bits*j);
    }
    return ret;
...
static ogg_int32_t _float32_unpack_tremolo(long val,int *point){
...
static int _determine_node_bytes_tremolo(long used, int leafwidth){
  ...
  if(_ilog_tremolo(3*used-6)+1 <= leafwidth*4)
...
/* convenience/clarity; leaves are specified as multiple of node word
   size (1 or 2) */
static int _determine_leaf_words_tremolo(int nodeb, int leafwidth){
...
static int _make_words_tremolo(char *l,long n,ogg_uint32_t *r,long quantvals,
		       codebook *b, oggpack_buffer *opb,int maptype){
  long i,j,count=0;
  long top=0;
  ...
  if(n<2){
	...
	}
	{
	  ...
	  r[chase*2+bit]= decpack_tremolo(i,count++,quantvals,b,opb,maptype) |
	    0x80000000;
	}
...
static int _make_decode_table_tremolo(codebook *s,char *lengthlist,long quantvals,
  ...
  if(s->dec_nodeb==4){
     ...
     * _make_words_tremolo without messing up length list looping.
    ...
    if(_make_words_tremolo(lengthlist,s->entries,
  ...
  if(_make_words_tremolo(lengthlist,s->entries,work,quantvals,s,opb,maptype))return 1;
...
long _book_maptype1_quantvals_tremolo(codebook *b){
  /* get us a starting hint, we'll polish it below */
  int bits=_ilog_tremolo(b->entries);
...
void vorbis_book_clear_tremolo(codebook *b){
...
int vorbis_book_unpack_tremolo(oggpack_buffer *opb,codebook *s){
  ...
  if(oggpack_read_tremolo(opb,24)!=0x564342)goto _eofout;

  /* first the basic parameters */
  s->dim=oggpack_read_tremolo(opb,16);
#if 0
  s->dec_buf=_ogg_malloc(sizeof(ogg_int32_t)*s->dim);
#else
  if(sizeof(ogg_int32_t)*s->dim < 0x7fffffffL)
    s->dec_buf=_ogg_malloc(sizeof(ogg_int32_t)*s->dim);
#endif
  if (s->dec_buf == NULL)
      goto _errout;
  s->entries=oggpack_read_tremolo(opb,24);
  if(s->entries<=0)goto _eofout;
  if(s->dim<=0)goto _eofout;
  if(_ilog_tremolo(s->dim)+_ilog_tremolo(s->entries)>24)goto _eofout;
  if (s->dim > INT_MAX/s->entries) goto _eofout;

  /* codeword ordering.... length ordered or unordered? */
  switch((int)oggpack_read_tremolo(opb,1)){
  case 0:
    /* unordered */
    ...
    if(oggpack_read_tremolo(opb,1)){
      /* yes, unused entries */

      for(i=0;i<s->entries;i++){
	if(oggpack_read_tremolo(opb,1)){
	  long num=oggpack_read_tremolo(opb,5);
  ...
  case 1:
    /* ordered */
    {
      long length=oggpack_read_tremolo(opb,5)+1;
      ...
      for(i=0;i<s->entries;){
	long num=oggpack_read_tremolo(opb,_ilog_tremolo(s->entries-i));
  ...
  if((maptype=oggpack_read_tremolo(opb,4))>0){
    s->q_min=_float32_unpack_tremolo(oggpack_read_tremolo(opb,32),&s->q_minp);
    s->q_del=_float32_unpack_tremolo(oggpack_read_tremolo(opb,32),&s->q_delp);
    s->q_bits=oggpack_read_tremolo(opb,4)+1;
    s->q_seq=oggpack_read_tremolo(opb,1);
  ...
  case 0:
    ...
    /* this is the correct boundary here; we lose one bit to
       node/leaf mark */
    s->dec_nodeb=_determine_node_bytes_tremolo(s->used_entries,_ilog_tremolo(s->entries)/8+1);
    s->dec_leafw=_determine_leaf_words_tremolo(s->dec_nodeb,_ilog_tremolo(s->entries)/8+1);
    s->dec_type=0;

    if(_make_decode_table_tremolo(s,lengthlist,quantvals,opb,maptype)) goto _errout;
    break;

  case 1:

    /* mapping type 1; implicit values by lattice  position */
    quantvals=_book_maptype1_quantvals_tremolo(s);
    ...
    /* dec_type choices here are 1,2; 3 doesn't make sense */
    {
      ...
      total2=(_ilog_tremolo(quantvals-1)*s->dim+8)/8+(s->q_bits+7)/8;
	...
	for(i=0;i<quantvals;i++)
	  ((ogg_uint16_t *)s->q_val)[i]=(ogg_uint16_t)oggpack_read_tremolo(opb,s->q_bits);

	if(oggpack_eop_tremolo(opb)){
	  s->q_val=0; /* cleanup must not free alloca memory */
	  goto _eofout;
	}

	s->dec_type=1;
	s->dec_nodeb=_determine_node_bytes_tremolo(s->used_entries,
					   (s->q_bits*s->dim+8)/8);
	s->dec_leafw=_determine_leaf_words_tremolo(s->dec_nodeb,
					   (s->q_bits*s->dim+8)/8);
	if(_make_decode_table_tremolo(s,lengthlist,quantvals,opb,maptype)){
	  s->q_val=0; /* cleanup must not free alloca memory */
	  goto _errout;
	}

	s->q_val=0; /* about to go out of scope; _make_decode_table_tremolo
                       was using it */
	...
	if(s->q_bits<=8){
	  ...
	  if (!s->q_val) goto _eofout;
	  for(i=0;i<quantvals;i++)
	    ((unsigned char *)s->q_val)[i]=(unsigned char)oggpack_read_tremolo(opb,s->q_bits);
	}else{
	...
	if(oggpack_eop_tremolo(opb))goto _eofout;

	s->q_pack=_ilog_tremolo(quantvals-1);
	s->dec_type=2;
	s->dec_nodeb=_determine_node_bytes_tremolo(s->used_entries,
					   (_ilog_tremolo(quantvals-1)*s->dim+8)/8);
	s->dec_leafw=_determine_leaf_words_tremolo(s->dec_nodeb,
					   (_ilog_tremolo(quantvals-1)*s->dim+8)/8);
	if(_make_decode_table_tremolo(s,lengthlist,quantvals,opb,maptype))goto _errout;
  ...
  case 2:
    ...
    if( (s->q_bits*s->dim+8)/8 <=4){ /* remember flag bit */
      ...
      s->dec_type=1;
      s->dec_nodeb=_determine_node_bytes_tremolo(s->used_entries,(s->q_bits*s->dim+8)/8);
      s->dec_leafw=_determine_leaf_words_tremolo(s->dec_nodeb,(s->q_bits*s->dim+8)/8);
      if(_make_decode_table_tremolo(s,lengthlist,quantvals,opb,maptype))goto _errout;
      ...
      s->dec_type=3;
      s->dec_nodeb=_determine_node_bytes_tremolo(s->used_entries,_ilog_tremolo(s->used_entries-1)/8+1);
      s->dec_leafw=_determine_leaf_words_tremolo(s->dec_nodeb,_ilog_tremolo(s->used_entries-1)/8+1);
      if(_make_decode_table_tremolo(s,lengthlist,quantvals,opb,maptype))goto _errout;
 ...
 _errout:
 _eofout:
  vorbis_book_clear_tremolo(s);
  return -1;
}

#ifndef ONLY_C
ogg_uint32_t decode_packed_entry_number_tremolo(codebook *book,
                                        oggpack_buffer *b);
#else
static inline ogg_uint32_t decode_packed_entry_number_tremolo(codebook *book,
						      oggpack_buffer *b){
  ogg_uint32_t chase=0;
  int  read=book->dec_maxlength;
  long lok = oggpack_look_tremolo(b,read),i;

  while(lok<0 && read>1)
    lok = oggpack_look_tremolo(b, --read);

  if(lok<0){
    oggpack_adv_tremolo(b,1); /* force eop */
    return -1;
  }
  ...
  if(i<read){
    oggpack_adv_tremolo(b,i+1);
    return chase;
  }
  oggpack_adv_tremolo(b,read+1);
  return(-1);
}
#endif

/* returns the [original, not compacted] entry number or -1 on eof *********/
long vorbis_book_decode_tremolo(codebook *book, oggpack_buffer *b){
  if(book->dec_type)return -1;
 return decode_packed_entry_number_tremolo(book,b);
}

#ifndef ONLY_C
int decode_map_tremolo(codebook *s, oggpack_buffer *b, ogg_int32_t *v, int point);
#else
static int decode_map_tremolo(codebook *s, oggpack_buffer *b, ogg_int32_t *v, int point){
  ogg_uint32_t entry = decode_packed_entry_number_tremolo(s,b);
  int i;
  if(oggpack_eop_tremolo(b))return(-1);
  ...
  case 3:{
    /* offset into array */
//    void *ptr= (void *)(s->q_val+entry*s->q_pack);
...
long vorbis_book_decodevs_add_tremolo(codebook *book,ogg_int32_t *a,
  ...
  if(book->used_entries>0){
    ...
    for (j=0;j<step;j++){
      if(decode_map_tremolo(book,b,v,point))return -1;
...
long vorbis_book_decodev_add_tremolo(codebook *book,ogg_int32_t *a,
			     oggpack_buffer *b,int n,int point){
  if(book->used_entries>0){
    ogg_int32_t *v = book->dec_buf;//(ogg_int32_t *)alloca(sizeof(*v)*book->dim);
    int i,j;
...
long vorbis_book_decodev_set_tremolo(codebook *book,ogg_int32_t *a,
			     oggpack_buffer *b,int n,int point){
  if(book->used_entries>0){
    ogg_int32_t *v = book->dec_buf;//(ogg_int32_t *)alloca(sizeof(*v)*book->dim);
    int i,j;
...
#ifndef ONLY_C
long vorbis_book_decodevv_add_tremolo(codebook *book,ogg_int32_t **a,
			      long offset,int ch,
			      oggpack_buffer *b,int n,int point);
#else
long vorbis_book_decodevv_add_tremolo(codebook *book,ogg_int32_t **a,
