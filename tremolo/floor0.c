...
static inline ogg_int32_t vorbis_invsqlook_i_tremolo(long a,long e){
...
static inline ogg_int32_t vorbis_fromdBlook_i_tremolo(long a){
...
static inline ogg_int32_t vorbis_coslook_i_tremolo(long a){
...
static inline ogg_int32_t vorbis_coslook2_i_tremolo(long a){
  int i=a>>COS_LOOKUP_I_SHIFT;
  int d=a&COS_LOOKUP_I_MASK;
...
static inline ogg_int32_t toBARK_tremolo(int n){
  int i;
#if 0
  for(i=0;i<54;i++)
...
void vorbis_lsp_to_curve_tremolo(ogg_int32_t *curve,int n,int ln,
  ...
  ogg_uint32_t inyq= (1UL<<31) / toBARK_tremolo(nyq);
  ogg_uint32_t imap= (1UL<<31) / ln;
  ogg_uint32_t tBnyq1 = toBARK_tremolo(nyq)<<1;
  ...
  for(i=0;i<m;i++){
    ...
    ilsp[i]=vorbis_coslook_i_tremolo(val);
  }
  ...
  while(i<n){
    ...
    wi=vorbis_coslook2_i_tremolo((map*imap)>>15);
    ...
    amp=vorbis_fromdBlook_i_tremolo(ampi*                     /*  n.4         */
			    vorbis_invsqlook_i_tremolo(qi,qexp)-
			                              /*  m.8, m+n<=8 */
			    ampoffseti);              /*  8.12[0]     */
...
void floor0_free_info_tremolo(vorbis_info_floor *i){
  vorbis_info_floor0 *info=(vorbis_info_floor0 *)i;
  if(info)_ogg_free(info);
}

vorbis_info_floor *floor0_info_unpack_tremolo (vorbis_info *vi,oggpack_buffer *opb){
  codec_setup_info     *ci=(codec_setup_info *)vi->codec_setup;
  int j;

  vorbis_info_floor0 *info=(vorbis_info_floor0 *)_ogg_malloc(sizeof(*info));
  info->order=oggpack_read_tremolo(opb,8);
  info->rate=oggpack_read_tremolo(opb,16);
  info->barkmap=oggpack_read_tremolo(opb,16);
  info->ampbits=oggpack_read_tremolo(opb,6);
  info->ampdB=oggpack_read_tremolo(opb,8);
  info->numbooks=oggpack_read_tremolo(opb,4)+1;
  ...
  for(j=0;j<info->numbooks;j++){
    info->books[j]=(char)oggpack_read_tremolo(opb,8);
    if(info->books[j]>=ci->books)goto err_out;
  }

  if(oggpack_eop_tremolo(opb))goto err_out;
  return(info);

 err_out:
  floor0_free_info_tremolo(info);
  return(NULL);
}

int floor0_memosize_tremolo(vorbis_info_floor *i){
  vorbis_info_floor0 *info=(vorbis_info_floor0 *)i;
  return info->order+1;
}

ogg_int32_t *floor0_inverse1_tremolo(vorbis_dsp_state *vd,vorbis_info_floor *i,
			     ogg_int32_t *lsp){
  vorbis_info_floor0 *info=(vorbis_info_floor0 *)i;
  int j,k;

  int ampraw=oggpack_read_tremolo(&vd->opb,info->ampbits);
  if(ampraw>0){ /* also handles the -1 out of data case */
    long maxval=(1<<info->ampbits)-1;
    int amp=((ampraw*info->ampdB)<<4)/maxval;
    int booknum=oggpack_read_tremolo(&vd->opb,_ilog_tremolo(info->numbooks));
	...
	if(vorbis_book_decodev_set_tremolo(b,lsp+j,&vd->opb,b->dim,-24)==-1)goto eop;
...
int floor0_inverse2_tremolo(vorbis_dsp_state *vd,vorbis_info_floor *i,
  ...
  if(lsp){
    ...
    vorbis_lsp_to_curve_tremolo(out,ci->blocksizes[vd->W]/2,info->barkmap,
