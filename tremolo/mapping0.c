...
void mapping_clear_info_tremolo(vorbis_info_mapping *info){
  if(info){
...
int mapping_info_unpack_tremolo(vorbis_info_mapping *info,vorbis_info *vi,
			oggpack_buffer *opb){
  int i;
  ...
  if(oggpack_read_tremolo(opb,1))
    info->submaps=oggpack_read_tremolo(opb,4)+1;
  else
    info->submaps=1;

  if(oggpack_read_tremolo(opb,1)){
    info->coupling_steps=oggpack_read_tremolo(opb,8)+1;
    info->coupling=
      _ogg_malloc(info->coupling_steps*sizeof(*info->coupling));
    
    for(i=0;i<info->coupling_steps;i++){
      int testM=info->coupling[i].mag=(unsigned char)(oggpack_read_tremolo(opb,ilog(vi->channels)));
      int testA=info->coupling[i].ang=(unsigned char)(oggpack_read_tremolo(opb,ilog(vi->channels)));
  ...
  if(oggpack_read_tremolo(opb,2)>0)goto err_out; /* 2,3:reserved */
    
  if(info->submaps>1){
    info->chmuxlist=_ogg_malloc(sizeof(*info->chmuxlist)*vi->channels);
    for(i=0;i<vi->channels;i++){
      info->chmuxlist[i]=(unsigned char)(oggpack_read_tremolo(opb,4));
  ...
  for(i=0;i<info->submaps;i++){
    int temp=oggpack_read_tremolo(opb,8);
    info->submaplist[i].floor=(char)oggpack_read_tremolo(opb,8);
    if(info->submaplist[i].floor>=ci->floors)goto err_out;
    info->submaplist[i].residue=(char)oggpack_read_tremolo(opb,8);
 ...
 err_out:
  mapping_clear_info_tremolo(info);
  return -1;
}

int mapping_inverse_tremolo(vorbis_dsp_state *vd,vorbis_info_mapping *info){
  ...
  for(i=0;i<vi->channels;i++){
    ...
    if(ci->floor_type[floorno]){
      ...
      floormemo[i]=alloca(sizeof(*floormemo[i])*
			  floor1_memosize_tremolo(ci->floor_param[floorno]));
      floormemo[i]=floor1_inverse1_tremolo(vd,ci->floor_param[floorno],floormemo[i]);
    }else{
      /* floor 0 */
      floormemo[i]=alloca(sizeof(*floormemo[i])*
			  floor0_memosize_tremolo(ci->floor_param[floorno]));
      floormemo[i]=floor0_inverse1_tremolo(vd,ci->floor_param[floorno],floormemo[i]);
    }
  ...
  for(i=0;i<info->submaps;i++){
    ...
    res_inverse_tremolo(vd,ci->residue_param+info->submaplist[i].residue,
  ...
  for(i=0;i<vi->channels;i++){
    ...
    if(ci->floor_type[floorno]){
      /* floor 1 */
      floor1_inverse2_tremolo(vd,ci->floor_param[floorno],floormemo[i],pcm);
    }else{
      /* floor 0 */
      floor0_inverse2_tremolo(vd,ci->floor_param[floorno],floormemo[i],pcm);
    }
  }
  ...
  /* only MDCT right now.... */
  for(i=0;i<vi->channels;i++)
    mdct_backward_tremolo(n,vd->work[i]);
