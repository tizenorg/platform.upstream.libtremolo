...
void res_clear_info_tremolo(vorbis_info_residue *info){
  if(info){
...
int res_unpack_tremolo(vorbis_info_residue *info,
		vorbis_info *vi,oggpack_buffer *opb){
  int j,k;
  codec_setup_info     *ci=(codec_setup_info *)vi->codec_setup;
  memset(info,0,sizeof(*info));

  info->type=oggpack_read_tremolo(opb,16);
  if(info->type>2 || info->type<0)goto errout;
  info->begin=oggpack_read_tremolo(opb,24);
  info->end=oggpack_read_tremolo(opb,24);
  info->grouping=oggpack_read_tremolo(opb,24)+1;
  info->partitions=(char)(oggpack_read_tremolo(opb,6)+1);
  info->groupbook=(unsigned char)oggpack_read_tremolo(opb,8);
  ...
  for(j=0;j<info->partitions;j++){
    int cascade=oggpack_read_tremolo(opb,3);
    if(oggpack_read_tremolo(opb,1))
      cascade|=(oggpack_read_tremolo(opb,5)<<3);
  ...
  for(j=0;j<info->partitions;j++){
	...
	unsigned char book=(unsigned char)oggpack_read_tremolo(opb,8);
  ...
  if(oggpack_eop_tremolo(opb))goto errout;

  return 0;
 errout:
  res_clear_info_tremolo(info);
  return 1;
}

int res_inverse_tremolo(vorbis_dsp_state *vd,vorbis_info_residue *info,
  ...
  if(info->type<2){
	...
	for(s=0;s<info->stages;s++){
	  ...
	  for(i=0;i<partvals;){
		...
		for(k=partitions_per_word-1;k>=0;k--)
	      ...
	      for(j=0;j<ch;j++){
		int temp=vorbis_book_decode_tremolo(phrasebook,&vd->opb);
		...
		if(info->stagemasks[(int)partword[j][i]]&(1<<s)){
		  ...
		  if(info->type){
		    if(vorbis_book_decodev_add_tremolo(stagebook,in[j]+offset,&vd->opb,
					       samples_per_partition,-8)==-1)
		      goto eopbreak;
		  }else{
		    if(vorbis_book_decodevs_add_tremolo(stagebook,in[j]+offset,&vd->opb,
						samples_per_partition,-8)==-1)
		      goto eopbreak;
		  }
		}
	...
	for(i=0;i<partvals;){
	  ...
	  if(s==0){
	    ...
	    temp=vorbis_book_decode_tremolo(phrasebook,&vd->opb);
	    if(temp==-1)goto eopbreak;
	  ...
	  for(k=0;k<partitions_per_word && i<partvals;k++,i++)
		...
		info->stagebooks[(partword[i]<<3)+s];
	      if(vorbis_book_decodevv_add_tremolo(stagebook,in,
