...
static DWORD run_test(void *tp)
{
  ...
  if(ov_open_tremolo(in, &vf, NULL, 0) < 0) {
  ...
  /* Throw the comments plus a few lines about the bitstream we're
     decoding */
  {
    char **ptr=ov_comment_tremolo(&vf,-1)->user_comments;
    vorbis_info *vi=ov_info_tremolo(&vf,-1);
    if (out != NULL)
    {
      while(*ptr){
        Output("%s\n",*ptr);
      ...
      Output("\nDecoded length: %ld samples\n",
             (long)ov_pcm_total_tremolo(&vf,-1));
      Output("Encoded by: %s\n\n",ov_comment_tremolo(&vf,-1)->vendor);
    }
  }

  while((!eof) && (max_samples > 0)){
    long ret=ov_read_tremolo(&vf,pcmout,sizeof(pcmout),&current_section);
    if (ret == 0) {
      /* EOF */
      eof=1;
    } else if (ret < 0) {
